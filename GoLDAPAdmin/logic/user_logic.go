...
// UpdateUserPwd 管理员更新用户密码
func (l UserLogic) UpdateUserPwd(c *gin.Context, req interface{}) (data interface{}, rspError interface{}) {
	r, ok := req.(*request.UpdateUserPwdReq)
	if !ok {
		return nil, ReqAssertErr
	}
	_ = c
	...
	// 校验工作
	filter := tools.H{"username": r.Username}
	...
	if int(minSort) != 1 {
		return nil, tools.NewValidatorError(fmt.Errorf("只有管理员才能修改用户密码"))
	}
